salesOrders = {
    initialize: function () {
        ...
        $("body").on("focusout, blur" 
                    , "#supplier_id, #location_id, #product_id, #quantity, #Cost, #InvoiceNumber" 
    ...
    editor: {
        events: {
            ...
            onEdit: function (e) {
                if (e.model.supplier_id === 0 && e.model.location_id === 0) {
                    var location_id = $("#main_location_id").data("kendoDropDownList").value();
                    var supplier_id = $("#main_supplier_id").data("kendoDropDownList").value();

                    e.model.location_id = location_id;
                    e.model.supplier_id = supplier_id;

                    $("#location_id").data("kendoDropDownList").bind("dataBound", function (ddl) { ddl.sender.value(location_id); })
                    $("#supplier_id").data("kendoDropDownList").bind("dataBound", function (ddl) { ddl.sender.value(supplier_id); })
                }
                ...
                $(uButton).click(function(e) {
                    setTimeout(function() {
                        var fields = ["supplier_id", "location_id", "product_id", "Cost" , "quantity", "Weight", "InvoiceNumber"]
