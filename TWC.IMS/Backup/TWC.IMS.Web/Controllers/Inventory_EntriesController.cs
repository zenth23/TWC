                ...
                using (var piBL = new BL.Product_Inventory(username))
                {
                    if (id != 0)
                    {
                        ...
                        if(dbObj != null)
                        {
                            ...
                            // REMOVE TO OLD
                            var piObj = await piBL.GetAsync(dbObj.product_id, dbObj.location_id, dbObj.supplier_id).ConfigureAwait(false);
                            if (piObj != null)
                            {
            ...
            try
            {
                ...
                using (var piBL = new BL.Product_Inventory(username))
                {
                    if (model.Id == 0)
                    {
                        ...
                        {
                            var piId = 0;
                            var piObj = await piBL.GetAsync(obj.product_id, obj.location_id, obj.supplier_id).ConfigureAwait(false);
                            if (piObj != null)
                            {
                    ...
                    else
                    {
                        ...
                        if (dbObj != null)
                        {
                            ...
                            if (!isSameEntry)
                            {
                                // ADD TO NEW
                                var piId = 0;
                                var piObj = await piBL.GetAsync(model.product_id, model.location_id, model.supplier_id).ConfigureAwait(false);
                                if (piObj != null)
                                {
                                ...
                                // REMOVE TO OLD
                                piObj = await piBL.GetAsync(dbObj.product_id, dbObj.location_id, dbObj.supplier_id).ConfigureAwait(false);
                                if (piObj != null)
                                {
                                    piObj.quantity = piObj.quantity - model.quantity;
                                    await piBL.UpdateAsync(piObj).ConfigureAwait(false);
                            ...
                            {
                                var piObj = await piBL.GetAsync(dbObj.product_id, dbObj.supplier_id, dbObj.supplier_id).ConfigureAwait(false);
                                if (piObj != null)
                                {
            ...
            try
            {
                using (var inventoryEntryBL = new BL.Inventory_Entry(User.Identity.Name))
                {
                    var includes = new string[] {
                        "Location", "Supplier", "Product_Master", "Category"
                    };
                ...
                using (var piBL = new BL.Product_Inventory(username))
                {
                    ...
                    foreach (var itemObj in inventoryEntries)
                    {
                        ...
                        if (productObj != null && catObj != null)
                        {
                            ...
                            obj.supplier_id = itemObj.SupplierId;
