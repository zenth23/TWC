                ...
                if (ModelState.IsValid)
                {
                    ...
                    using (_passwordHistoriesBL = new PasswordHistories(User.Identity.Name))
                    {
                        ...
                        if (model.Id == 0)
                        {
                            ...
                            string username = await TWC.IMS.Common.StandardUsernameGenerator.GenerateUsernameAsync(model.PersonalModel.FirstName, model.PersonalModel.LastName, model.PersonalModel.MiddleName, model.PersonalModel.Suffix, model.PersonalModel.Nickname).ConfigureAwait(false);
                            ...
                            string tempPassword = await GenerateTemporaryPasswordAsync().ConfigureAwait(false);
