...
namespace TWC.IMS.Web.Controllers
{
    ...
    {
        [CustomAuthorize(AccessName = "ProductMasters.CanView")]
        ...
        public async Task<ActionResult> SaveProduct([DataSourceRequest] DataSourceRequest request, ProductsViewModel model)
        {
            ...
            try
            {
                using (_productsBL = new BL.Products(username))
                {
                    ...
                    {
                        if (model.Id == 0 && HttpContext.HasPermission("ProductMasters.CanAdd"))
                        {
                            var objGetByCode = await _productsBL.GetByNameAsync(model.product_name).ConfigureAwait(false);
                            ...
                            if (objGetByCode == null)
                            {
                                ...
                                ProdInvObj.gemstones = model.gemstones;
                                ProdInvObj.karat = model.karat;
                                ProdInvObj.material = model.material;
                                ProdInvObj.product_name = model.product_name;
                                ProdInvObj.ProductType_id = model.product_type;
                                ProdInvObj.retail_price = model.retail_price;
                                ProdInvObj.selling_price = model.selling_price;
                                //ProdInvObj.weight = model.weight;
                        ...
                        else if (model.Id != 0 && HttpContext.HasPermission("ProductMasters.CanEdit"))
                        {
                            var bussList = await _productsBL.GetListAsync().ConfigureAwait(false);
                            var obj = bussList.FirstOrDefault(b => b.Id == model.Id);

                            if (bussList.FirstOrDefault(b => b.Id != model.Id && b.product_name == model.product_name) != null)
                                return Json(new { Success = false, Message = "Product already exists!" }, JsonRequestBehavior.AllowGet);
                            ...
                            if (obj != null)
                            {
                                ...
                                obj.ProductType_id = model.product_type;
        ...
        public async Task<ActionResult> GetProduct([DataSourceRequest]DataSourceRequest request)
        {
            try
            {
                using (_productsBL = new BL.Products(User.Identity.Name))
                {
                    var units = await _productsBL.GetListAsync("Product_Type").ConfigureAwait(false);
                    ...
                    foreach (var unit in units)
                    {
                        var model = new ProductsViewModel()
                        {
                            ...
                            product_type = unit.ProductType_id,
                            ProductTypeDescription = unit.Product_Type?.Description,
                            karat = unit.karat,
                            //weight = unit.weight,
                            ...
                            LowStockThreshold = unit.LowStockThreshold.HasValue ? (int)unit.LowStockThreshold.Value : 0
        ...
        public async Task<ActionResult> GetImage(int? id)
        {
            try
            {
                if (id.HasValue)
                {
                    using (var bl = new BL.Product_Master_Image(User.Identity.Name))
                    {
                        var imageObj = await bl.GetAsync(id.Value);
                        if (imageObj != null)
                        {
                        ...
                        {
                            var filePath = Server.MapPath("~/Content/Images/No-Image.png");
