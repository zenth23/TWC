...
namespace TWC.IMS.Web.Controllers
{
    ...
    public class SalesOrdersController : BaseController
    {
        ...
        [CustomAuthorize(AccessName = "SalesOrders.CanView")]
        ...
        [CustomAuthorize(AccessName = "SalesOrders.CanDelete")]
        [HttpPost]
        ...
        [HttpPost]
        public async Task<ActionResult> Save_back([DataSourceRequest] DataSourceRequest request, SalesOrderViewModel model)
        {
            if (model == null || model.location_id <= 0)
            {
                return Json(new { Success = false, Message = "Invalid Location. Please select a valid location." });
                    ...
                    using (var package = new ExcelPackage(excelFile.InputStream))
                    {
                        ...
                        for (var row = 2; row <= rowCount; row++)
                        {
                            ...
                            itemObj.Amount = itemObj.IsGold ? (itemObj.Weight ?? 0) * itemObj.Cost
                                    : itemObj.Cost * itemObj.quantity;//itemObj.quantity * itemObj.Cost;
        ...
        [CustomAuthorize(AccessName = "SalesOrders.CanAdd")]
        [ValidateAntiForgeryToken]
        [HttpPost]
