...
namespace TWC.IMS.Web.Controllers
{
    ...
    public class CommonController : BaseController
    {
        ...
        [SkipLogActionFilter]
        public async Task<JsonResult> GetSalesTypesForDdl(int? id = null)
        {
            var username = User.Identity.Name;
            using (var bl = new BL.SalesTypes(username))
            {
                var list = new List<Models.SalesTypeViewModel>();
                if(id.HasValue)
                {
                    var dbObj = await bl.GetAsync(id.Value).ConfigureAwait(false);
                    if (dbObj != null)
                        list.Add(new Models.SalesTypeViewModel
                        {
                            Id = dbObj.Id,
                            SalesType_name = dbObj.SalesType_name
                        });
                }
                else
                {
                    list = (await bl.GetListAsync().ConfigureAwait(false))
                                .Select(x => new Models.SalesTypeViewModel
                                {
                                    Id = x.Id,
                                    SalesType_name = x.SalesType_name
                                }).ToList();
                }
        ...
        [SkipLogActionFilter]
        public async Task<JsonResult> GetProductTypesForDdl(int? id = null)
        {
            var username = User.Identity.Name;
            using (var bl = new BL.producttypes(username))
            {
                var list = new List<TWC.IMS.Models.ProductType>();
                if (id.HasValue)
                {
                    var dbObj = await bl.GetAsync(id.Value).ConfigureAwait(false);
                    if (dbObj != null)
                ...
                {
                    list = (await bl.GetListAsync().ConfigureAwait(false)).ToList();
                }


                return Json(list.Select(x => new { x.Id, x.Description }), JsonRequestBehavior.AllowGet);
            }
        }
