...
productmaster = {
    initialize: function () {
        //productmaster.editor.events.onKendoGridRowValidate()
    ...
    editor: {
        events: {
            onKendoGridEdit: function (e) {
                ...
                //let ProductMasterId = $(`#Id`).val()
                //let model = e.model;
                ////if (model.isNew()) {
                //    e.container.find("input[name=ProductMasterUser_ProductMaster]").val(ProductMasterId).trigger("change")
                //}
            },
            ...
            onGridAddClick: function (e) {
                ...
                productmaster.editor.events.onOpenModal(id)
            ...
            onOpenModal: function (id) {
                ...
                productmaster.editor.events.onGridUserLoad(id)
            ...
            onGridUserLoad: function (id) {
                dataSource = new kendo.data.DataSource({
                    transport: {
                        read: {
                            url: `${window.rootUrl}product_master/GetApplicationsUser?id=${id}`,
                            dataType: "jsonp"
                        },
                        update: {
                            url: `${window.rootUrl}product_master/UpdateApplicationUsers`,
                            dataType: "jsonp"
                        },
                ...
                $("#grid").kendoGrid({
                    ...
                    toolbar: ["save", "cancel"],
                    columns: [
                        { template: '#=productmaster.editor.events.onDirtyField(data,"IsSelected")#<input type="checkbox" #= IsSelected ? \'checked="checked"\' : "" # class="chkbx k-checkbox k-checkbox-md k-rounded-md" />', title: "IsSelected", width: 110, attributes: { class: "k-text-center" } },
                        "ApplicationCode",
                        "ApplicationName",
                    ],
                    editable: true
        ...
        services: {
            deleteProductMaster: function (data) {
                ...
                let url = `${window.rootUrl}product_master/deletesubscriberuser`;
            ...
            getUserApplications: function (data) {
                ...
                let url = `${window.rootUrl}product_master/getuserapplications`;
                return $.post(url, data);
            }
        }
    },
