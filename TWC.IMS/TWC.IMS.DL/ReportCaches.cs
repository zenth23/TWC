    ...
    public class ReportCaches : IDisposable
    {
        ...
        public async Task<Models.ReportCache> GetAsync(Guid reportId)
        {
            try
            {
                ...
                {
                    var obj = await db.ReportCaches.AsNoTracking()
                                                   .FirstOrDefaultAsync(a => a.UniqueKey == reportId)
        ...
        public async Task<Models.ReportCache> GetLastAsync(string fileName)
        {
            try
            {
                ...
                {
                    var obj = await db.ReportCaches.AsNoTracking()
                                                   .Where(x => x.ReportName == fileName && x.CreatedBy == _username)
        ...
        public async Task<int> InsertAsync(Models.ReportCache obj)
        {
        ...
        public async Task<int> DeleteAsync(Guid reportId)
        {
            ...
            using (var db = new Models.Entities())
            {
                try
                {
                    var obj = await db.ReportCaches.FirstOrDefaultAsync(a => a.UniqueKey == reportId).ConfigureAwait(false);
                    if (obj != null)
                    {
                        db.ReportCaches.Remove(obj);
        ...
        public async Task<int> DeleteAsync(string reportName)
        {
            string username = _username;
            using (var db = new Models.Entities())
            {
                try
                {
                    var obj = await db.ReportCaches.FirstOrDefaultAsync(a => a.ReportName == reportName &&
                                                                             a.CreatedBy == _username)
                                                   .ConfigureAwait(false);
                    if (obj != null)
                    {
                        db.ReportCaches.Remove(obj);
