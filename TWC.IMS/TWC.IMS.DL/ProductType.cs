...
namespace TWC.IMS.DL
{
    public class ProductType : IDisposable
    {
        ...
        public async Task<IEnumerable<Models.ProductType>> GetListAsync(params string[] includeEntities)
        {
            try
            {
                using (var db = new Models.Entities())
                {
                    var query = (DbQuery<Models.ProductType>)db.Set<Models.ProductType>();
        ...
        public async Task<Models.ProductType> GetAsync(int id ,params string[] includeEntities)
        {
            try
            {
                using (var db = new Models.Entities())
        ...
        public async Task<Models.ProductType> GetByNameAsync(string productypeName)
        {
            try
            {
                using (var db = new Models.Entities())
                {
                    var obj = await db.Product_Type.AsNoTracking()
                                                  .AsQueryable()
                                                  .Where(a => string.Compare(a.Code, productypeName.Trim(), true) == 0)
                                                  .FirstOrDefaultAsync()
                                                  .ConfigureAwait(false);
                    return obj;
                }
        ...
        public async Task<int> InsertAsync(Models.ProductType obj)
        {
        ...
        public async Task<int> UpdateAsync(Models.ProductType obj)
        {
            string username = _username;
            try
            {
        ...
        public async Task<int> DeleteAsync(int id)
        {
            ...
            using (var db = new Models.Entities())
            {
                try
                {
                    var obj = await db.Product_Type.FindAsync(id).ConfigureAwait(false);
                    if (obj != null)
                    {
        ...
        public async Task<int> DeleteAsync(Guid uniqueKey)
        {
            ...
            using (var db = new Models.Entities())
            {
                try
                {
                    var obj = await db.Product_Type.FirstOrDefaultAsync(a => a.UniqueKey == uniqueKey).ConfigureAwait(false);
                    if (obj != null)
                    {
                        db.Product_Type.Remove(obj);
                        return await db.SaveChangesAsync(username).ConfigureAwait(false);
