    ...
    public class Locations : IDisposable
    {
        ...
        public async Task<Models.Location> GetByNameAsync(string SalesTypeName)
        {
            try
            {
                using (var db = new Models.Entities())
                {
                    var obj = await db.Locations.AsNoTracking()
                                                  .AsQueryable()
                                                  .Where(a => string.Compare(a.location_name, SalesTypeName.Trim(), true) == 0)
                                                  .FirstOrDefaultAsync()
                                                  .ConfigureAwait(false);
                    return obj;
                }
        ...
        public async Task<int> DeleteAsync(int id)
        {
            ...
            using (var db = new Models.Entities())
            {
                try
                {
                    var obj = await db.Locations.FindAsync(id).ConfigureAwait(false);
                    if (obj != null)
                    {
        ...
        public async Task<int> DeleteAsync(Guid uniqueKey)
        {
            ...
            using (var db = new Models.Entities())
            {
                try
                {
                    var obj = await db.Locations.FirstOrDefaultAsync(a => a.UniqueKey == uniqueKey).ConfigureAwait(false);
                    if (obj != null)
                    {
                        db.Locations.Remove(obj);
                        return await db.SaveChangesAsync(username).ConfigureAwait(false);
