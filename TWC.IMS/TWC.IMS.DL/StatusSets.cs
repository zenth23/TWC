...
namespace TWC.IMS.DL
{
    public class StatusSets : IDisposable
    {
        ...
        public async Task<IEnumerable<Models.StatusSet>> GetListAsync()
        {
            try
            {
                using (var db = new Models.Entities())
                {
                    var query = from p in db.StatusSets.AsNoTracking().AsQueryable()
                                orderby p.Name
                                select p;
        ...
        public async Task<IEnumerable<Models.StatusSet>> GetUserAccountStatusListAsync()
        {
            try
            {
                using (var db = new Models.Entities())
                {
                    var query = from p in db.StatusSets.AsNoTracking().AsQueryable()
                                where p.Module.Contains("USERACCOUNT")
                                orderby p.Name
                                select p;
        ...
        public async Task<Models.StatusSet> GetAsync(int id)
        {
            try
            {
                using (var db = new Models.Entities())
                {
                    var obj = await db.StatusSets.FindAsync(id).ConfigureAwait(false);
                    return obj;
                }
            }
            catch (Exception ex)
        ...
        public async Task<Models.StatusSet> GetAsync(Guid uniqueKey)
        {
            try
            {
                using (var db = new Models.Entities())
                {
                    var obj = await db.StatusSets.AsNoTracking()
                                                 .AsQueryable()
        ...
        public async Task<Models.StatusSet> GetAsync(string name)
        {
            try
            {
                using (var db = new Models.Entities())
        ...
        public async Task<int> InsertAsync(Models.StatusSet obj)
        {
        ...
        public async Task<int> UpdateAsync(Models.StatusSet obj)
        {
            string username = _username;
            try
            {
        ...
        public async Task<int> DeleteAsync(Guid uniqueKey)
        {
            ...
            using (var db = new Models.Entities())
            {
                try
                {
                    var obj = await db.StatusSets.FirstOrDefaultAsync(a => a.UniqueKey == uniqueKey).ConfigureAwait(false);
                    if (obj != null)
                    {
                        db.StatusSets.Remove(obj);
                        return await db.SaveChangesAsync(username).ConfigureAwait(false);
