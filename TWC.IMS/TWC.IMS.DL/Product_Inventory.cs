...
namespace TWC.IMS.DL
{
    public class Product_Inventory : IDisposable
    {
        ...
        public async Task<Models.Product_Inventory> GetAsync(int invId, params string[] includeEntities)
        {
            try
            {
                using (var db = new Models.Entities())
                {
                    ...
                    return await query.AsNoTracking()
                                      .FirstOrDefaultAsync(x => x.Id == invId)
                                      .ConfigureAwait(false);
                }
            }
            catch (Exception ex)
        ...
        public async Task<Models.Product_Inventory> GetAsync(int product_id, int location_id, params string[] includeEntities)
        {
            try
            {
                using (var db = new Models.Entities())
                {
                    ...
                    return await query.AsNoTracking()
                                      .FirstOrDefaultAsync(x =>  x.location_id == location_id
