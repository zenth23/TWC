...
{
    public class VideoMaintenanceBL : IDisposable
    {
        private readonly string _username;
        private VideoMaintenanceDL _dataLayer;

        public VideoMaintenanceBL(string username)
        {
            if (string.IsNullOrWhiteSpace(username))
                throw new NullReferenceException(TWC.IMS.Common.Messages.MUST_LOGIN_FIRST);

            _username = username;
            _dataLayer = new VideoMaintenanceDL(_username);
        }

        public async Task<IEnumerable<VideoMaintenance>> GetListAsync(params string[] includes)
        {
            try
            {
        ...
        public async Task<VideoMaintenance> GetAsync(int id, params string[] includes)
        {
            try
            {
        ...
        public async Task UpdateAsync(VideoMaintenance item)
        {
            try
            {
