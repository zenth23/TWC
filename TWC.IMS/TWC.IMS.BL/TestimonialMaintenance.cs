...
{
    public class TestimonialMaintenanceBL : IDisposable
    {
        private readonly string _username;
        private TestimonialMaintenanceDL _dataLayer;

        public TestimonialMaintenanceBL(string username)
        {
            if (string.IsNullOrWhiteSpace(username))
                throw new NullReferenceException(TWC.IMS.Common.Messages.MUST_LOGIN_FIRST);

            _username = username;
            _dataLayer = new TestimonialMaintenanceDL(_username);
        }

        public async Task<IEnumerable<TestimonialMaintenance>> GetListAsync(params string[] includes)
        {
            try
            {
        ...
        public async Task<TestimonialMaintenance> GetAsync(int id, params string[] includes)
        {
            try
            {
        ...
        public async Task UpdateAsync(TestimonialMaintenance item)
        {
            try
            {
