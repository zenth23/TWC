...
{
    public class CarouselMaintenanceBL : IDisposable
    {
        private readonly string _username;
        private CarouselMaintenanceDL _dataLayer;

        public CarouselMaintenanceBL(string username)
        {
            if (string.IsNullOrWhiteSpace(username))
                throw new NullReferenceException(TWC.IMS.Common.Messages.MUST_LOGIN_FIRST);

            _username = username;
            _dataLayer = new CarouselMaintenanceDL(_username);
        }

        public async Task<IEnumerable<CarouselMaintenance>> GetListAsync(params string[] includes)
        {
            try
            {
        ...
        public async Task<CarouselMaintenance> GetAsync(int id, params string[] includes)
        {
            try
            {
        ...
        public async Task UpdateAsync(CarouselMaintenance item)
        {
            try
            {
